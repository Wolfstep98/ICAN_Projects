    ...
    protected void LateUpdate() 
    {
        ...
        {  
            ...
            if (rayCastHit != null && rayCastHit.Length > 0)
            {
                ...
                {
                    UnityEngine.Vector3 candidatePosToTarget = rayCastHit[1].point - this.transform.position;
                    float candidatePosToTargetLength = candidatePosToTarget.magnitude;
                    if (candidatePosToTargetLength < posToTargetLength)
                    {
                        raycastGO = rayCastHit[i].collider.gameObject;
                        posToTarget = candidatePosToTarget;
                        posToTargetLength = candidatePosToTargetLength;
                    }
                }
