...
public class AbstractPool : MonoBehaviour
{
    ...
    [SerializeField]
    private AbstractPoolEntity[] entities = null;
    public AbstractPoolEntity[] Entities { get { return this.entities; } }
    #endregion
    ...
    public void Create()
    {
        if (this.entity != null)
        {
            if (!this.poolerReady)
            {
                this.entities = new AbstractPoolEntity[this.numberOfEntities];
                for (int i = 0; i < this.numberOfEntities; i++)
                {
                    GameObject obj = Object.Instantiate<GameObject>(this.entity, Vector3.right * i, Quaternion.identity);
                    obj.name = this.entity.name + "_" + i;
                    obj.SetActive(false);
                    this.entities[i] = obj.GetComponent<AbstractPoolEntity>();
    ...
    public void Sleep(AbstractPoolEntity entity)
    {
        if (this.poolerReady)
        {
            entity.GoToSleep();
    ...
    public void Clean()
    {
        if (this.poolerReady)
        {
            if (this.entities != null)
            {
                ...
                {
                    Object.Destroy(this.entities[i].gameObject);
